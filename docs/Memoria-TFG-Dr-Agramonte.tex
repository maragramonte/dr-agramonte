...
\textbf{Coste.} El desarrollo es un coste \emph{único} (este propio TFG), y la operación es deliberadamente barata gracias a las decisiones de arquitectura: un frontend estático sin dependencias, un único backend Java y una base de datos relacional caben de sobra en una VPS modesta (del orden de 10\,€/mes). No hay comisiones por cita ni cuotas de plataforma de terceros.
...
Los objetivos del proyecto se formularon con un criterio que ha guiado toda la memoria: que cada meta fuera \textbf{concreta, verificable y realmente alcanzable} dentro del alcance de un TFG. No se persigue prometer un producto enorme a medias, sino entregar un sistema más pequeño pero completo, coherente y demostrable.
...
\begin{enumerate}
  ...
  \item Validar la calidad con pruebas automatizadas y análisis de vulnerabilidades, y preparar la defensa con una demo reproducible.
  \item Ofrecer un \textbf{cuadro de mando de gestión} (módulo SGE) con analítica agregada —carga por médico, demanda por especialidad, reparto por centro y tasa de cancelación— restringido a los roles \texttt{MEDICO}/\texttt{ADMIN}.
\end{enumerate}
...
% ----------------------------------------------------------------------------
\section{Contextualización del sector}
% ----------------------------------------------------------------------------

\subsection{Descripción del sector profesional}

El ciclo de Desarrollo de Aplicaciones Multiplataforma (DAM) aporta competencias en desarrollo multiplataforma, bases de datos, programación de servicios, diseño de interfaces y despliegue. Este proyecto las aplica a un ámbito muy concreto: el sanitario privado, y dentro de él, a la gestión de agenda, la relación paciente--médico y la comunicación entre ambos.

El puesto de referencia para este TFG es el de \emph{desarrollador/a de aplicaciones} que da soporte a un consultorio de medicina interna. Es un perfil mixto: por un lado, trabajo de oficina propio del desarrollo de software (análisis, programación, pruebas, documentación); por otro, la necesidad de entender la realidad de un entorno clínico, donde los datos que se manejan son sensibles, los usuarios tienen edades y niveles digitales muy distintos, y un error no es un simple \emph{bug}, sino una persona afectada. Comprender ese sector —sus tiempos, sus prioridades y sus límites legales— es tan parte del trabajo como escribir el código, y de hecho condiciona casi todas las decisiones técnicas que aparecen más adelante en esta memoria.

\subsection{Tipos de solución y dónde encaja el proyecto}
...
\subsection{Competencias del desarrollador DAM aplicadas}

El proyecto no nace de un módulo concreto, sino de la integración de las competencias del ciclo, que es precisamente lo que se espera de un perfil de \emph{desarrollo}:

\begin{itemize}
  \item \textbf{Acceso a datos:} modelo relacional, JPA y transacciones (las tablas y el bloqueo de la reserva).
  \item \textbf{Programación de servicios y procesos:} la API REST, la seguridad y la tarea programada de recordatorios.
  \item \textbf{Desarrollo de interfaces:} el frontend accesible y la PWA.
  \item \textbf{Sistemas y despliegue:} contenedores Docker, nginx y perfiles de configuración.
  \item \textbf{Empresa e iniciativa emprendedora:} el análisis del hueco de mercado y la viabilidad económica.
\end{itemize}

Las salidas naturales de este perfil —desarrollo \emph{backend}, \emph{full-stack} o iniciación a \emph{DevOps}— se ven reflejadas en las distintas capas que ha sido necesario construir para este sistema.

\subsection{Riesgos laborales}

\begin{itemize}
  \item \textbf{Ergonómicos:} posturas prolongadas ante pantalla, fatiga visual y movimientos repetitivos al teclear, tanto en el puesto de desarrollo como en la consulta.
  \item \textbf{Psicosociales:} estrés por interrupciones y carga administrativa; la digitalización de citas reduce esa presión.
  \item \textbf{Seguridad de la información:} el tratamiento de datos de salud (categoría especial RGPD) implica riesgo de fuga o acceso indebido.
  \item \textbf{Medioambientales:} consumo energético de equipos y servidores; gestión responsable de residuos electrónicos (RAEE).
\end{itemize}

\subsection{Medidas preventivas}

\begin{itemize}
  \item \textbf{Ergonomía:} silla regulable, pantalla a la altura de los ojos, regla 20-20-20 para la vista y pausas activas periódicas.
  \item \textbf{Organización:} la propia herramienta reduce el riesgo psicosocial al eliminar llamadas e interrupciones de agenda.
  \item \textbf{Protección de datos:} cifrado en tránsito (HTTPS), contraseñas con \texttt{BCrypt}, secretos en variables de entorno, control de accesos por rol y copias de seguridad de la base de datos.
  \item \textbf{Sostenibilidad:} despliegue en contenedores ligeros y apagado de servicios no usados.
\end{itemize}

\subsection{Equipos de protección y buenas prácticas (EPIs)}

En un puesto de desarrollo software los EPIs físicos son limitados, por lo que la prevención se apoya en \textbf{equipamiento ergonómico} (reposamuñecas, filtro de luz azul, iluminación adecuada) y, sobre todo, en \textbf{buenas prácticas de seguridad} como ``EPI digital'': no commitear credenciales, principio de mínimo privilegio, actualización de dependencias y revisión de vulnerabilidades.

% ----------------------------------------------------------------------------
\section{Desarrollo principal}
% ----------------------------------------------------------------------------

\subsection{Del boceto inicial a la implementación}
...
Comparando el boceto con el resultado final se aprecia que las ideas centrales sobrevivieron al desarrollo (reserva por pasos, selección de centro, identidad visual del consultorio), mientras que otras se ajustaron a lo que el alcance de un TFG permite construir con garantías. Documentar esa evolución es parte del valor del proyecto: enseña no solo qué se hizo, sino por qué se decidió así.
...
\input{tikz-diagrama-arquitectura}

\subsection{Organización del backend en capas y DTOs}
...
\subsection{Justificación de las tecnologías}
...
\begin{itemize}
  ...
  \item \textbf{Flyway.} Versiona el esquema en migraciones numeradas (V1 a V6), de modo que cualquier entorno —el de desarrollo, el del tribunal o uno futuro en producción— se reconstruye exactamente igual al arrancar.
...
\begin{itemize}
  \item \texttt{usuarios}: identidad y credenciales (email único, contraseña cifrada, nombre, teléfono, rol). El campo \texttt{rol} discrimina entre \texttt{PACIENTE}, \texttt{MEDICO} y \texttt{ADMIN}, y \texttt{cuenta\_invitada} marca las cuentas creadas en una reserva sin registro.
...
\subsection{Frontend y paleta visual}
...
Tras validar, se crea la \texttt{Cita}, se marca el horario no disponible y se invoca \texttt{CitaNotificationService}. El control de concurrencia es \textbf{bloqueo pesimista} (\texttt{SELECT \ldots FOR UPDATE} sobre \texttt{Horario}): si dos pacientes reservan la misma franja, una operación gana y la otra recibe \textbf{409 Conflict}, de forma determinista y sin reintentos.

\input{tikz-diagrama-reserva}

\subsection{Reserva como invitado}
...
\subsection{Notificaciones Twilio}
...
\paragraph{Selección de centro (multi-ubicación).} El flujo de reserva permite elegir entre dos centros antes de fijar la hora: \emph{Clínica Salamanca} (Madrid) y \emph{Consulta Palma de Mallorca}, cada uno con sus propios días de atención. Esto refleja una realidad habitual: un mismo profesional atiende en más de una ubicación. El centro \textbf{se persiste en la base de datos}: existe una entidad \texttt{Centro} (tabla \texttt{centros}, creada en la migración Flyway V5) con un \texttt{codigo} estable —\texttt{madrid}, \texttt{palma}— que coincide con el identificador que ya empleaba el frontend, de modo que no hubo que reescribir la web. El backend expone el listado mediante \texttt{GET /api/centros}.

\paragraph{Agenda del médico por centro.} La multi-ubicación se llevó hasta el final: cada \texttt{horario} pertenece a un centro (columna \texttt{centro\_id}, migración V6), repartiendo la agenda del médico por día de la semana —Madrid los lunes, martes y jueves; Palma los miércoles, viernes y sábados— de forma que cada día el profesional está en un único sitio. A partir de ahí, todo encaja de manera coherente:
\begin{itemize}
  \item La \textbf{disponibilidad se filtra por centro}: \texttt{GET /api/disponibilidad?medicoId=\&fecha=\&centroCodigo=} solo devuelve los huecos de ese centro (un miércoles no ofrece horas de Madrid).
...
\subsection{Accesibilidad y usabilidad (WCAG)}
...
\subsection{Sincronización de citas: una sola fuente de verdad}
...
\midrule
No doble reserva concurrente & \texttt{ReservaPublicaIntegrationTest} & Integración & 409 \checkmark \\
\rowcolor{rowgray}
...
\begin{enumerate}
  \item \emph{Modelo de datos en PostgreSQL con Flyway.} \textbf{Cumplido:} seis tablas, migraciones V1--V6 y perfiles \texttt{postgres}/\texttt{mysql}.
  ...
  \item \emph{Integridad de reservas con \texttt{SELECT FOR UPDATE}.} \textbf{Cumplido y verificado:} la prueba de integración confirma que la segunda reserva concurrente recibe 409.
  ...
  \item \emph{Notificaciones por Twilio con recordatorio programado.} \textbf{Cumplido:} avisos a médico y paciente y recordatorio a $\sim$24\,h vía \texttt{@Scheduled}.
  \item \emph{Despliegue reproducible con Docker Compose.} \textbf{Cumplido:} un único comando levanta base de datos, backend y frontend.
...
\textbf{Limitaciones honestas:} la cobertura no está medida con JaCoCo (se valida cualitativamente por ramas críticas), el panel médico no expone API de historial, no hay refresh token ni escenario de réplicas/failover. Son líneas de evolución natural del TFG, no carencias del alcance entregado.
...
\begin{itemize}
  ...
  \item \textbf{Seguridad y escalado:} \emph{refresh token}, despliegue en la nube bajo el dominio ya registrado \texttt{dragramonte.com} con certificado HTTPS, y réplicas de base de datos.
...
\textbf{Aprendizajes:} el proyecto ha consolidado competencias de los módulos de DAM (acceso a datos, programación de servicios, interfaces, sistemas de gestión empresarial y empresa e iniciativa emprendedora) en un caso real, integrando seguridad, concurrencia y despliegue.

\textbf{Reflexión:} la coherencia entre memoria, código y demo oral es tan importante como la funcionalidad; este documento describe la versión real del sistema a junio de 2026.
...
\section*{Anexo A. Checklist de cumplimiento (rúbricas del módulo)}

\input{checklist-rubricas}

\section*{Anexo B. Manual de usuario (resumen)}

\begin{enumerate}
  \item \textbf{Registro / acceso:} desde \texttt{login.html}, crear cuenta (rol PACIENTE) o iniciar sesión. El sistema devuelve un token JWT que el navegador guarda para las peticiones protegidas.
  ...
  \item \textbf{Mis citas:} consultar las citas propias y su estado; el paciente recibe confirmación (y recordatorio $\sim$24\,h antes) por Twilio si tiene teléfono.
...
\section*{Anexo C. Capturas de pantalla}

Incluir aquí las capturas de la demo (inicio, servicios, flujo de reserva, mis citas y modo oscuro). Cada captura debe ir numerada y con su leyenda. Ejemplo de inclusión:

\begin{verbatim}
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{capturas/reserva-paso1.png}
  \caption{Primer paso del flujo de reserva: selección de centro.}
\end{figure}
\end{verbatim}
