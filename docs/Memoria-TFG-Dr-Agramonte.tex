...
\begin{itemize}
  ...
  \item \textbf{Flyway.} Versiona el esquema en migraciones numeradas (V1 a V8), de modo que cualquier entorno —el de desarrollo, el del tribunal o uno futuro en producción— se reconstruye exactamente igual al arrancar.
...
\begin{enumerate}
  \item \emph{Modelo de datos en PostgreSQL con Flyway.} \textbf{Cumplido:} seis tablas, migraciones V1--V8 y perfiles \texttt{postgres}/\texttt{mysql}.
  ...
  \item \emph{Despliegue reproducible con Docker Compose.} \textbf{Cumplido:} un único comando levanta base de datos, backend y frontend; y, más allá del objetivo, la aplicación está \textbf{desplegada en la nube} (Railway) y es pública con HTTPS en \url{https://www.dragramonte.com}.
...
\begin{itemize}
  ...
  \item \textbf{Seguridad y escalado:} \emph{refresh token}, réplicas de base de datos con \emph{failover} y un plan de alojamiento de mayor garantía (la beta actual corre sobre la capa de prueba de Railway).
