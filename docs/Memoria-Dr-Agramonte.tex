...
\paragraph{Demo pública.} Junto al despliegue real existe una \textbf{demo pública} en \url{https://maragramonte.github.io/dr-agramonte/}, pensada para que cualquiera pueda probar la aplicación entera sin credenciales, sin instalar nada y sin tocar los datos reales de la consulta. No es una maqueta aparte: son las mismas páginas y el mismo cliente de API que el frontend de producción, acompañados de una \textbf{API simulada que se ejecuta en el propio navegador} y que intercepta las llamadas a \texttt{/api/\ldots} devolviendo los mismos contratos JSON que los DTOs del backend. Reproduce también sus reglas de negocio: el reparto de la agenda por centro de la migración V6, el 409 cuando la franja ya está ocupada y el 401/403 según el rol del usuario. Los datos son ficticios y viven en el \texttt{localStorage} del visitante, de modo que cada persona trabaja sobre su propia copia y nada sale de su navegador; el canal de Telegram se representa con un chat simulado dentro de la página, con los mismos textos que genera \texttt{CitaNotificationService}, sin llegar a usar el bot real.

La demo se genera con \texttt{scripts/build-demo.sh}, que copia el frontend e inyecta la API simulada \textbf{sin modificar} el directorio \texttt{frontend/}: la imagen de producción nunca incluye el modo demo. Un flujo de GitHub Actions la construye en cada cambio que la afecte —también en los \emph{pull request}, donde solo se construye y no se publica— y la despliega en GitHub Pages, y todas sus páginas se marcan \texttt{noindex} para no competir en buscadores con el dominio real. Conviene dejar escrito su límite: al no haber base de datos detrás, la demo \emph{no} puede demostrar la concurrencia real —el bloqueo pesimista solo existe contra PostgreSQL—, que es justamente lo que cubren las pruebas de integración.
...
\begin{enumerate}
  ...
  \item \emph{Despliegue reproducible con Docker Compose.} \textbf{Cumplido:} un único comando levanta base de datos, backend y frontend; y, más allá del objetivo, la aplicación está \textbf{desplegada en un servidor propio} (VPS con Docker y Caddy) y es pública con HTTPS en \url{https://www.dragramonte.com}, con una \textbf{demo pública} en GitHub Pages que permite probarla sin credenciales ni servidor.
